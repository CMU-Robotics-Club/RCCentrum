...
class OrderedFlatpageNode(template.Node):
    ...
    def render(self, context):
        ...
        flatpages = OrderedFlatPage.objects.filter(sites__id=site_pk)
...
@register.tag
def get_ordered_flatpages(parser, token):
    """
    ...
    Syntax::
        ...
        {% get_ordered_flatpages ['url_starts_with'] [for user] as context_name %}

    Example usage::

        {% get_ordered_flatpages as flatpages %}
        {% get_ordered_flatpages for someuser as flatpages %}
        {% get_ordered_flatpages '/about/' as about_pages %}
        {% get_ordered_flatpages prefix as about_pages %}
        {% get_ordered_flatpages '/about/' for someuser as about_pages %}
    """
    ...
    if len(bits) >= 3 and len(bits) <= 6:
        ...
        return OrderedFlatpageNode(context_name, starts_with=prefix, user=user)
    else:
        raise template.TemplateSyntaxError(syntax_message)